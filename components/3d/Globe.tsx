...
export default function Globe({
    ...
    useEffect(() => {
        ...
        const updateMarkers = () => {
            ...
            markerMeshes = [];
            rippleMeshes.forEach((mesh) => globeGroup.remove(mesh));
            rippleMeshes = [];
            if (markerConfig.markers && markerConfig.markers.length > 0) {
                ...
                const rippleGeometry = new RingGeometry(1, 1.6, 48);
                markerConfig.markers.forEach((marker, index) => {
                    if (
                        !marker ||
                    ...
                    markerMesh.userData = { name: marker.name ?? "", index };
                    ...
                    rippleMesh.scale.setScalar(markerSize);
                    rippleMesh.position.copy(worldPosition);
                    rippleMesh.quaternion.setFromUnitVectors(
                        new Vector3(0, 0, 1),
                        outwardNormal
                    );
                    rippleMesh.position.addScaledVector(outwardNormal, 0.001);
        ...
        const animate = () => {
            ...
            const isRippling = rippleMeshes.length > 0;
            if (isRippling) {
                rippleClock += 0.016;
                const cycleDuration = 1.8;
                const staggerPerMarker = cycleDuration / rippleMeshes.length;
                const isHoveringAnyMarker = hoveredMarkerIndex !== null;
                rippleMeshes.forEach((rippleMesh, index) => {
                    const phase = index * staggerPerMarker;
                    const cycleT =
                        ((rippleClock + phase) % cycleDuration) /
                        cycleDuration;
                    const isHovered = hoveredMarkerIndex === index;
                    const maxScale = isHovered ? 3.6 : 2.4;
                    const maxOpacity = isHovered
                        ? 0.7
                        : isHoveringAnyMarker
                          ? 0.15
                          : 0.4;
                    const rippleScale = mapLinear(cycleT, 0, 1, 1, maxScale);
                    rippleMesh.scale.setScalar(
                        0.01 * markerRadiusMultiplier * rippleScale
                    );
                    (rippleMesh.material as MeshBasicMaterial).opacity =
                        mapLinear(cycleT, 0, 1, maxOpacity, 0);
                });
                needsRender = true;
            }
        ...
        const handleMouseDown = (event: MouseEvent) => {
            ...
            const handleMouseUp = () => {
                ...
                isDragging = false;
                canvas.style.cursor =
                    hoveredMarkerIndex !== null ? "pointer" : "grab";
                // Settle the globe back to the equator so it stays neat
                // after the user is done spinning it around.
                velocity.y = 0;
                targetRotation.y = 0;
                startAnimation();
            };
        ...
        const handleMouseMove = (event: MouseEvent) => {
            ...
            if (stopOnHover) {
                ...
                isHovering = oceanIntersects.length > 0;
            }

            const markerIntersects = raycaster.intersectObjects(markerMeshes);
            if (markerIntersects.length > 0) {
                const data = markerIntersects[0].object.userData as {
                    name?: string;
                    index?: number;
                };
                const nextIndex =
        ...
        canvas.addEventListener("mouseleave", clearHoveredMarker);
        ...
        return () => {
            ...
            canvas.removeEventListener("mouseleave", clearHoveredMarker);
